...
from projects.mmdet3d_plugin.sgn.modules.sdb import SDB, SDBv2
...
class SGNHeadStage2(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.sdb = SDBv2(channel=self.embed_dims, out_channel=self.embed_dims//2, num_classes=self.n_classes)
    ...
    def forward(self, mlvl_feats, img_metas, target):
        ...
        vox_feats_diff, x, x_san, logit = self.sdb(vox_feats_flatten.permute(3, 0, 1, 2).unsqueeze(0)) # 1, C,H,W,Z
    ...
    def step(self, out_dict, target, img_metas, step_type):
        ...
        if step_type== "train":
            ...
            pred, mid_ined, mid_ori = out_dict['san_dict']['logit'], out_dict['san_dict']['x_san'], out_dict['san_dict']['x']
            loss_dict['loss_san'] = self.sdb.san.cal_loss(pred, mid_ined, target_2, mid_ori, class_weight)
