...
class SGNHeadOED(nn.Module):
    ...
    def forward(self, mlvl_feats, img_metas, target):
        ...
        out = self.ssc_header(complete_feats)
        out["seed_sem_logit"] = seed_aux_out
        out["seed_coords"] = seed_coords
    ...
    def step(self, out_dict, target, img_metas, step_type):
        ...
        if step_type== "train":
            ...
            seed_pred = out_dict["seed_sem_logit"]
            seed_coords = out_dict['seed_coords']
            seed_target_2 = target_2.clone()[0, seed_coords[:, 0], seed_coords[:, 1], seed_coords[:, 2]]
            ...
            loss_seed = lovasz_softmax(F.softmax(seed_pred, dim=1), seed_target_2, ignore=255)
            loss_seed += F.cross_entropy(seed_pred, seed_target_2.long(), ignore_index=255)
            loss_dict['loss_seed'] = loss_seed
