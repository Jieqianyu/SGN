...
class SGNHeadLite(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.sparse_header = builder.build_head(sparse_header_dict)
    ...
    def forward(self, mlvl_feats, mlvl_depths, img_metas, target):
        ...
        """
        out = {}
        x3d = self.flosp([mlvl_feats[-1]], img_metas) # bs, c, nq
        bs, c, _ = x3d.shape
        ...
        # Load proposals
        sparse_out = self.sparse_header(mlvl_feats, mlvl_depths, img_metas, target)
        out['sparse_out'] = sparse_out

        proposal = None
        if 'output_2' in sparse_out.keys():
            proposal = sparse_out[f'output_2'].dense().detach().cpu().numpy()
            proposal =  proposal > 0
        
        if proposal is None or proposal.sum() <= 1:
            proposal = np.ones(self.bev_h, self.bev_w, self.bev_z)
    ...
    def step(self, out_dict, target, img_metas, step_type):
        ...
        if step_type== "train":
            ...
            loss_dict.update(self.sparse_header.training_step(out_dict['sparse_out'], target, img_metas))
