...
class LiteMono(nn.Module):
    ...
    def __init__(self, in_chans=3, model='lite-mono', height=192, width=640,
        ...
        if model == 'lite-mono':
            ...
            elif height == 384 and width == 1280:
                self.dilation = [[1, 2, 5], [1, 2, 5], [1, 2, 5, 1, 2, 5, 2, 4, 10]]
        ...
        if kwargs.get('pretrained', None) is not None:
            encoder_dict = torch.load(kwargs['pretrained'], map_location='cpu')
            print('load pretrained ckpt for encoder')
            self.load_state_dict({k: v for k, v in encoder_dict.items() if k in self.state_dict()})
